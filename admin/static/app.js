...
const state = {
  ...
  chartInstances: {},
  // Dynamic form lists
  inclusions: ['Hotel stay included', 'Daily continental breakfast', 'Airport transfers'],
  highlights: ['Fully guided local tours', 'Scenic sunset views'],
  itinerary: ['', '', '', '', ''] // Default for 5 days
};
...
const elements = {
  ...
  addTripCloseBtn: document.getElementById('add-trip-close-btn'),
  addTripCancelBtn: document.getElementById('add-trip-cancel-btn'),
  tripForm: document.getElementById('trip-create-form'),
  formError: document.getElementById('form-error'),
  formSuccess: document.getElementById('form-success'),
  btnGlobalAdd: document.getElementById('add-trip-global-btn'),
  btnTabAdd: document.getElementById('add-trip-btn-tab'),
  tripDaysInput: document.getElementById('trip-days'),
  inclusionsContainer: document.getElementById('inclusions-container'),
  ...
  btnAddInclusion: document.getElementById('btn-add-inclusion'),
...
function initApp() {
  ...
  state.token = localStorage.getItem('voyageiq-admin-token');
  if (state.token) {
    try {
  ...
  const openTripModal = () => {
    resetTripForm();
    elements.addTripModal.classList.add('active');
    renderDynamicFormLists();
    lucide.createIcons();
  };
  elements.btnGlobalAdd.addEventListener('click', openTripModal);
  elements.btnTabAdd.addEventListener('click', openTripModal);
  
  const closeTripModal = () => {
    elements.addTripModal.classList.remove('active');
  };
  elements.addTripCloseBtn.addEventListener('click', closeTripModal);
  elements.addTripCancelBtn.addEventListener('click', closeTripModal);
  elements.addTripModal.addEventListener('click', (e) => {
    if (e.target === elements.addTripModal) closeTripModal();
  });

  // Dynamic form array triggers
  elements.tripDaysInput.addEventListener('input', handleDurationChange);
  elements.btnAddInclusion.addEventListener('click', () => {
    state.inclusions.push('');
    renderInclusions();
  });
  elements.btnAddHighlight.addEventListener('click', () => {
...
function showLogin() {
  elements.dashboardWrapper.classList.remove('active');
  elements.loginWrapper.classList.add('active');
}

function showDashboard() {
  elements.loginWrapper.classList.remove('active');
  elements.dashboardWrapper.classList.add('active');
  
  // Set User Details
  if (state.user) {
    document.getElementById('admin-name').textContent = state.user.name || 'Organizer Portal';
    document.getElementById('admin-initial').textContent = (state.user.name || 'A')[0].toUpperCase();
    const roleEl = document.querySelector('.user-role');
    if (roleEl) roleEl.textContent = state.user.role === 'admin' ? 'Super Admin' : 'Trip Organizer';
  }
...
async function handleLogin(e) {
  ...
  try {
    ...
    if (!response.ok) {
      elements.loginError.textContent = data.detail || 'Authorization failed.';
      elements.loginError.style.display = 'block';
      return;
    }
    
    // Store token
    localStorage.setItem('voyageiq-admin-token', data.token);
    state.token = data.token;
    state.user = data.user;
    
    showDashboard();
  } catch (err) {
    elements.loginError.textContent = 'Connection to database backend failed.';
    elements.loginError.style.display = 'block';
  }
}

function logout() {
  localStorage.removeItem('voyageiq-admin-token');
  state.token = null;
  state.user = null;
  showLogin();
}
...
function switchTab(tabName) {
  ...
  // Show global "Add Trip" button in top navbar only when viewing Trips tab
  if (tabName === 'trips') {
    elements.btnGlobalAdd.style.display = 'inline-flex';
  } else {
    elements.btnGlobalAdd.style.display = 'none';
  }

  // Update Page Header Info
  const tabTitles = {
    overview: { title: 'Overview', sub: 'Real-time vacation planner performance & insights' },
    bookings: { title: 'Bookings Log', sub: 'Manage and inspect traveler trip packages reservation list' },
    trips: { title: 'Trips Catalog', sub: 'Analyze individual trip performances, revenues and ratings' },
...
async function fetchAllData() {
  ...
  try {
    ...
    const [overviewRes, chartsRes, tablesRes] = await Promise.all([
      ...
      fetch('/api/analytics/charts', { headers }),
      fetch('/api/analytics/tables', { headers })
    ]);

    if (overviewRes.status === 401 || chartsRes.status === 401 || tablesRes.status === 401) {
      logout();
      return;
    }
...
function renderCharts() {
  ...
  state.chartInstances.trends = new Chart(trendsCtx, {
    type: 'bar',
    data: {
      labels: chartData.booking_trends.map(t => t.month),
      datasets: [
        {
          label: 'Revenue (₹)',
          data: chartData.booking_trends.map(t => t.revenue),
        ...
        },
        {
          ...
          data: chartData.booking_trends.map(t => t.bookings),
          type: 'line',
          borderColor: '#8b5cf6',
  ...
  state.chartInstances.categories = new Chart(categoriesCtx, {
    type: 'doughnut',
    data: {
      labels: chartData.category_revenue.map(c => c.category),
      datasets: [{
        data: chartData.category_revenue.map(c => c.revenue),
        backgroundColor: [
  ...
  state.chartInstances.styles = new Chart(stylesCtx, {
    type: 'bar',
    data: {
      labels: chartData.ai_styles.map(s => s.style),
      datasets: [{
        data: chartData.ai_styles.map(s => s.count),
  ...
  state.chartInstances.destinations = new Chart(destCtx, {
    type: 'bar',
    data: {
      labels: chartData.popular_destinations.map(d => d.destination),
      datasets: [{
        data: chartData.popular_destinations.map(d => d.count),
  ...
  doublyFiltered.forEach(t => {
    ...
    tr.innerHTML = `
      ...
      <td>
        ...
        ${t.is_featured ? '<span class="badge badge-accent" style="font-size:0.55rem;padding:1px 4px;margin-top:2px;display:inline-block;">Featured</span>' : ''}
      </td>
...
function openAIPlanModal(p) {
  ...
  elements.modalBody.innerHTML = `
    ...
    <div class="detail-item">
      ...
      <div class="pre-wrapped" style="font-family:monospace; font-size:0.8rem; line-height:1.4;">${p.generated_plan || 'Empty.'}</div>
    </div>
  `;
  openModal();
}
...
function renderReviewsTable() {
  ...
  filtered.forEach(r => {
    ...
    for (let i = 1; i <= 5; i++) {
      if (i <= r.rating) starsHtml += '<i data-lucide="star"></i>';
      else starsHtml += '<span class="empty-star"><i data-lucide="star"></i></span>';
    }
...
// ================= DYNAMIC DURATION & FORM BUILDER =================
function resetTripForm() {
  state.inclusions = ['Hotel stay included', 'Daily continental breakfast', 'Airport transfers'];
  state.highlights = ['Fully guided local tours', 'Scenic sunset views'];
  
  const days = 5;
  elements.tripDaysInput.value = days;
  state.itinerary = Array.from({ length: days }, () => '');
...
function handleDurationChange(e) {
  ...
  state.itinerary = Array.from({ length: days }, (_, i) => currentItinerary[i] || '');
  
  document.getElementById('itinerary-days-indicator').textContent = days;
...
function renderInclusions() {
  elements.inclusionsContainer.innerHTML = '';
  state.inclusions.forEach((val, idx) => {
    const row = document.createElement('div');
    row.className = 'dynamic-input-row';
    row.innerHTML = `
      <input type="text" class="trip-inclusion-input" required placeholder="e.g. 5-Star Hotel Accommodations" value="${val}">
      <button type="button" class="btn-icon-delete" onclick="removeInclusionItem(${idx})">
        <i data-lucide="trash-2"></i>
      </button>
    `;
    elements.inclusionsContainer.appendChild(row);
  });
  lucide.createIcons();
}

window.removeInclusionItem = function(index) {
  state.inclusions.splice(index, 1);
  renderInclusions();
};

function renderHighlights() {
  elements.highlightsContainer.innerHTML = '';
  state.highlights.forEach((val, idx) => {
    const row = document.createElement('div');
    row.className = 'dynamic-input-row';
    row.innerHTML = `
      <input type="text" class="trip-highlight-input" required placeholder="e.g. Private beach dinners" value="${val}">
      <button type="button" class="btn-icon-delete" onclick="removeHighlightItem(${idx})">
        <i data-lucide="trash-2"></i>
      </button>
    `;
    elements.highlightsContainer.appendChild(row);
  });
  lucide.createIcons();
}

window.removeHighlightItem = function(index) {
...
function renderItineraryInputs() {
  ...
  state.itinerary.forEach((val, idx) => {
    const row = document.createElement('div');
    row.className = 'itinerary-day-row';
    row.innerHTML = `
      <span class="day-num-badge">Day ${idx + 1} Schedule</span>
      <textarea class="trip-itinerary-input" required placeholder="Detail the schedule for Day ${idx + 1}...">${val}</textarea>
    `;
    elements.itineraryContainer.appendChild(row);
  });
}
...
async function handleTripSubmit(e) {
  ...
  // Read inclusions inputs
  const inclusions = Array.from(document.querySelectorAll('.trip-inclusion-input')).map(input => input.value.trim());
  const highlights = Array.from(document.querySelectorAll('.trip-highlight-input')).map(input => input.value.trim());
  const itinerary = Array.from(document.querySelectorAll('.trip-itinerary-input')).map(input => input.value.trim());

  const days = parseInt(elements.tripDaysInput.value);
  const payload = {
    ...
    price_rupees: parseInt(document.getElementById('trip-price').value),
    image_url: document.getElementById('trip-image').value.trim(),
    short_description: document.getElementById('trip-short-desc').value.trim(),
    full_description: document.getElementById('trip-full-desc').value.trim(),
    inclusions,
    highlights,
    itinerary
  };
  ...
  try {
    ...
    const data = await response.json();
    if (!response.ok) {
      elements.formError.textContent = data.detail || 'Failed to create trip.';
      elements.formError.style.display = 'block';
      return;
    }

    elements.formSuccess.textContent = 'Trip package successfully published!';
    ...
    elements.formError.textContent = 'Database request failed.';
    elements.formError.style.display = 'block';
  }
}
